...
CDLRFHandler::CDLRFHandler(void)
{
}
	...
	)
{
	...
	return runWithParameter(0,literalNum, dimensionValuesStr,vOdd, matrixCells, mutantsNeed, 
...
{
	vector<int>literalNum;
	return runWithParameter(0,literalNum, dimensionValuesStr,vOdd, matrixCells, allOTPs,  allUTPs);
}
//运行DLRF算法  param为1，只返回validShrink部分  param为2，只返回expand部分 其他值两个都返回
string CDLRFHandler::runWithParameter(int param, vector<int>&literalNum, string dimensionValuesStr, bool vOdd, vector<vector<CCell*>>& matrixCells, vector<int> allOTPs, vector<int> allUTPs)
{
...
string CDLRFHandler::runWithParameter(int param,
									  vector<int>&literalNum, 
									  string dimensionValuesStr,
									  bool vOdd, 
									  vector<vector<CCell*>>& matrixCells,
	...
	for(int i=0; i<dimensionNum; i++)
	{
		...
		for(int j=0; j<removedDimensionValues.size(); j++)
		{
			...
			*/
			...
			{
				//一行
				vector<CCell*> matrixRowsshrink;
				for(int m=0; m<removedCells.size(); m++)
				{
					matrixRowsshrink.push_back(removedCells.at(m));
				}
				if(param!=2)
					matrixCells.push_back(matrixRowsshrink);
				validShrinksCount++;
				//continue;
			}
			// equivalent LIF 
				...
				case 2://这种情况下扩充的是剩下的两个，比如原来为1,2，扩充部分就为3,4
					if(k!=i&&biggestValue==4)//1.k==i时expand之后和shrink之前是一样的;2.如果该维的最大取值只到2则也不能进行expand
					{
						...
						*/
						...
						// hasEquivalentLifOccurred && mutantsNeed && EquivalentLifOccurred_involved_inserted_literal_not_from_single-literal-term
						//if (mutantsNeed && hasEquivalentLifOccurred)
						//{
			...
			//已经获取所有的expand cells，将扩充的一半合并组成矩阵中的一行
			for(int m=0; m<expandCells.size(); m++)
			{
				vector<CCell*> matrixRowexpand;
				/*for(int a=0; a<removedCells.size(); a++)
					matrixRowexpand.push_back(removedCells.at(a));*/
				for(int b=0; b<expandCells.at(m).size(); b++)
					matrixRowexpand.push_back(expandCells.at(m).at(b));
				if (param!=1)
					matrixCells.push_back(matrixRowexpand);
			}
		}
	}
