...
string CLRFHandler::runWithParameter(int param, vector<int>&literalNum, string dimensionValuesStr, bool vOdd, vector<vector<CCell*>>& matrixCells, vector<int> allOTPs, vector<int> allUTPs)
{
...
//运行LRF算法  param为1，只返回validShrink部分  param为2，只返回expand部分 其他值两个都返回
string CLRFHandler::runWithParameter(int param,
									  vector<int>&literalNum, 
									  string dimensionValuesStr,
									  bool vOdd, 
									  vector<vector<CCell*>>& matrixCells,
	...
	for(int i=0; i<dimensionNum; i++)
	{
		...
		for(int j=0; j<removedDimensionValues.size(); j++)
		{
			...
			*/
			...
			//有剩余点
			if(!mutantsNeed && removedCells.size() > 0)
			{
				//一行
				vector<CCell*> matrixRow;
				for(int m=0; m<removedCells.size(); m++)
