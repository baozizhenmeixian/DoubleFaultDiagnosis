...
CDoubleLRRFHandler::CDoubleLRRFHandler(void)
{
}
...
string CDoubleLRRFHandler::runWithParameter(
				...
				for(l=0; l<removedDimensionValues1.size(); l++)
				{
					...
					*/
					...
					for(n=0; n<dimensionNum; n++)
					{
						...
						vector<vector<int>> remainedDimensionValues2;
						vector<int> remainedTemp2;
						...
						case 2://这种情况下扩充的是剩下的两个，比如原来为1,2，扩充部分就为3,4
							if((n!=k&&n!=i&&biggestValue==4)/*||(n!=i&&biggestValue==4)*/)//1.n==k时expand之后和shrink之前是一样的;2.如果该维的最大取值只到2则也不能进行expand
							{
								...
								{
									remainedTemp2.clear();
									remainedTemp2.push_back(1);
									remainedTemp2.push_back(2);
									remainedTemp2.push_back(3);
									remainedTemp2.push_back(4);
									remainedDimensionValues2.push_back(remainedTemp2);
						...
						case 1://这种情况下，有两种扩充方式，比如原来为1，扩充部分可以是4或2，
							...
							int firstNum = removedDimensionValues.at(j).at(0);
							bool noPlus1 = false;
							bool noMinus1 = false;
							if(n==k)
							{
							...
							else if(n == i)
							{
								...
								{
									if(m_commonFuncs.boundValue(num+1,biggestValue)==firstNum)
										noPlus1 = true;
									if(m_commonFuncs.boundValue(num-1,biggestValue)==firstNum)
										noMinus1 = true;
								}
							...
							else 
							{
								...
								{
									if(num==1)//如果该维最大值是2，那么num只可能是1或2?
										noMinus1=true;
									else
										noPlus1=true;
								}
							...
							if(!noPlus1)
							{
								...
								{
									remainedTemp2.clear();
									remainedTemp2.push_back(num);
									remainedTemp2.push_back(m_commonFuncs.boundValue(num+1,biggestValue));
									remainedDimensionValues2.push_back(remainedTemp2);
